...
if __name__ == "__main__":
     ...
     for doc in corpus_tfidf:
          if iter_com < len(global_list.comment_scores):
               ...
               # 1.为避免score大于
               # 5，需要将单词的分数对应扩大缩小。这个比例应该由单词的score和与总分（5）确定。
               # 2.不应该使用单词的weightprop作为扩大缩小比例的依据，因为此概率之和始终为1。
               for  single in doc:
               ...
               if int(global_list.comment_scores[iter_com]) > 0 and weight_sum > 0:
                    print "origin score :"
                    print int(global_list.comment_scores[iter_com])
                    print "predict score :"
                    print  sum / weight_sum
                    print "Precision loss: "
                    print  (int(global_list.comment_scores[iter_com]) - sum / weight_sum) / int(
                         global_list.comment_scores[iter_com])
